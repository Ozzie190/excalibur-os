// ─── IRON PROTOCOL — Workout Tracker ─────────────────────────
// Core app logic: state, rendering, actions
// Pattern mirrors BioHack OS: global state S, loadS/saveS, draw()
...
var S = {
  ...
  // Active workout
  activeWorkout: null,
  // Shape: { id, templateId, name, startedAt, exercises: [{ exerciseId, sets: [{ weight, reps, type, completed, rpe, timestamp }], restSeconds, notes }] }

  // Rest timer
  restTimer: null,
  // Shape: { startedAt (ms), duration (seconds), exerciseId, remaining (seconds) }
  ...
  // Templates
  templates: [],
  // Shape: [{ id, name, icon, color, exercises: [{ exerciseId, sets, repTarget, restSeconds }], scheduledDays: {mon:false,...} }]

  // Personal Records
  prs: {},
  // exerciseId -> { e1rm: {weight, date}, maxWeight: {weight, reps, date}, maxVolume: {volume, date} }
  ...
  // Soreness check-ins: { workoutId: { date, level (1-5), notes } }
  sorenessLog: {},
...
function loadS() {
  try {
    var sv = localStorage.getItem("ip_v1");
    if (sv) {
      var p = JSON.parse(sv);
      ["activeWorkout","restTimer","workoutHistory","templates","prs","bodyWeight",
       "measurements","sorenessLog","units","defaultRestSeconds","vibrateOnTimerEnd","autoStartTimer",
       "selectedGoal"].forEach(function(k) {
...
function saveS() {
  try {
    localStorage.setItem("ip_v1", JSON.stringify({
      activeWorkout:S.activeWorkout, workoutHistory:S.workoutHistory,
      templates:S.templates, prs:S.prs, bodyWeight:S.bodyWeight,
      measurements:S.measurements, sorenessLog:S.sorenessLog, units:S.units,
      defaultRestSeconds:S.defaultRestSeconds, vibrateOnTimerEnd:S.vibrateOnTimerEnd,
      autoStartTimer:S.autoStartTimer, selectedGoal:S.selectedGoal
    }));
...
function svgRing(pct, color, sz) {
  sz=sz||44; var r=(sz-8)/2, circ=2*Math.PI*r;
  var off=circ-(Math.min(100,Math.max(0,pct))/100)*circ;
  return '<svg width="'+sz+'" height="'+sz+'" class="ring"><circle cx="'+(sz/2)+'" cy="'+(sz/2)+'" r="'+r+'" fill="none" stroke="#222" stroke-width="4"/><circle cx="'+(sz/2)+'" cy="'+(sz/2)+'" r="'+r+'" fill="none" stroke="'+color+'" stroke-width="4" stroke-dasharray="'+circ.toFixed(1)+'" stroke-dashoffset="'+off.toFixed(1)+'" style="stroke-linecap:round;transition:stroke-dashoffset .5s"/></svg>';
}
...
try {
  timerWorker = new Worker('rest-timer.js');
...
function updateTimerDisplay() {
  ...
  root.innerHTML = '<div class="timer-bar" style="'+(isAlmostDone?'border-top-color:#38b000;':'')+'">'+
    '<div class="timer-ring-wrap">' + svgRing(pct, color, 44) +
...
function showToast(message, duration) {
  ...
  el.style.cssText = 'position:fixed;bottom:70px;left:50%;transform:translateX(-50%);background:#1a1a2e;color:#e0e0f0;padding:10px 20px;border-radius:8px;font-size:12px;z-index:600;border:1px solid #2a2a2e;opacity:0;transition:opacity .3s;pointer-events:none;font-family:DM Mono,monospace';
  document.body.appendChild(el);
  requestAnimationFrame(function(){ el.style.opacity = '1'; });
  setTimeout(function(){ el.style.opacity = '0'; setTimeout(function(){ if (el.parentNode) el.remove(); }, 300); }, duration);
}
...
function navigateToExerciseDetail(exerciseId) {
  ...
  S.filterEquip = '';
  document.querySelectorAll('.tab').forEach(function(t){ t.classList.remove('active'); });
  var el = document.getElementById('t-exercises');
  if (el) el.classList.add('active');
  draw();
  setTimeout(function() {
...
function renderWorkoutStart() {
  ...
  muscles.forEach(function(mid) {
    ...
    h += svgRing(rec.pct, rec.color, 32);
...
function toggleExercise(id) {
  ...
  if (S.tab === 'exercises') { updateExerciseResults(); } else { draw(); }
}
...
// Update duration display every second (no full redraw — just text node)
setInterval(function() {
  if (!S.activeWorkout || S.tab !== 'workout') return;
...
// Full redraw every 60s to keep suggestion bars + volume current
// Skip if modal is open (would destroy the modal and steal focus)
setInterval(function() {
  if (S.activeWorkout && S.tab === 'workout' && !document.querySelector('.modal-bg')) draw();
}, 60000);
